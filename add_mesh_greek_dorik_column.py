...
def create_circle(segments, radius, z):
    '''
...
def create_faces(segments, size_vertices, faces, step=0):
    '''
...
def greek_doric_column_mesh(self, context):
    '''
    ...
    # Dodajemy do listy wierzchołki necking
    ...
    # Tworzymy nowy mesh
    mesh = bpy.data.meshes.new("Greek Doric Column")
...
class AddGreekDoricColumn(bpy.types.Operator,  object_utils.AddObjectHelper):
    '''
        Klasa 
    '''
    # id po którym będzie wywoływać się klasa 
    bl_idname = "mesh.primitive_greekdorikcolumn_add"
    # Nazwa klasy
    bl_label = "Greek Doric Column"
    # Opis klasy
    bl_description = "Construct a step greek doric column mesh"
    # Opcje które posiada klasa
    bl_options = {'REGISTER', 'UNDO', 'PRESET'}

    # Zmiena przechowywająca imie
    GreekColumn : BoolProperty(name = "Greek Doric Column",
                default = True,
                description = "Greek Doric Column")
    # Zmiena która sprawdza czy parametry były zmienione
    change : BoolProperty(name = "Change",
                default = False,
                description = "change Greek Doric Column")
    ...
    def execute(self, context):
        ...
        if bpy.context.mode == "OBJECT":
            if context.selected_objects != [] and context.active_object and \
                (context.active_object.data is not None) and ('Greek Doric Column' in context.active_object.data.keys()) and \
                (self.change == True):
                ...
                obj.data = greek_doric_column_mesh(self, context)
                ...
                # Tworzymy kolumne 
                mesh = greek_doric_column_mesh(self, context)
                # Tworzymy obiekt oraz przpisujemy dane 
                obj = object_utils.object_data_add(context, mesh, operator=self)
            
            obj.data["Greek Doric Column"] = True
            obj.data["change"] = False
            # Pobieramy parametry
            for prm in GreekDoricColumnParameters():
...
def GreekDoricColumnParameters():
    '''
        Funkcja zwacająca parametry
    '''
    GreekDoricColumnParameters = [
        "segments",
        "radius",
        "depth",
        ]
    return GreekDoricColumnParameters
